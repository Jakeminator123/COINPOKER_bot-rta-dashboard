...
export default function Tooltip({ content, children, delay = 800, position = 'top' }: TooltipProps) {
        ...
        {isVisible && (
          <motion.div
            ...
            style={{
              ...
              transform: `translateX(${position === 'left' ? '-100%' : position === 'right' ? '0%' : '-50%'}) translateY(${position === 'top' ? '-100%' : position === 'bottom' ? '0%' : '-50%'})`,
              pointerEvents: 'auto' as const,
            }}
            onMouseEnter={() => setIsVisible(true)}
            onMouseLeave={() => setIsVisible(false)}
          >
            <div className="bg-slate-900/95 backdrop-blur-sm border border-slate-700 rounded-lg px-4 py-3 shadow-xl max-w-xs">
