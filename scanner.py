...
class CoinPokerScanner:
        ...
        try:
            ...
            if hwnd and pid:
                ...
                try:
                    ...
                    print(f"[Scanner] Could not bring window to front: {e}")
                
                print("[Scanner] Starting nickname detector...")
    ...
    def _bring_window_to_front(self, hwnd: int) -> bool:
        """
        Bring CoinPoker window to foreground for reliable OCR capture.
        ...
        try:
            ...
            time.sleep(0.3)  # Wait for window to be fully visible
            ...
            print(f"[Scanner] Window focus error: {e}")
            return False
...
def _save_config_to_disk(config: dict[str, Any]) -> bool:
    """Save config to config.txt on disk (next to .exe or in project root)."""
    try:
        import sys
        from pathlib import Path
        ...
        # Build config.txt content
        lines = [
            "# ====================================",
            "# Bot Detection System Configuration",
            "# Generated by S+K+I Config GUI",
            "# ====================================",
            "",
        ]
        
        # Ensure RAM_CONFIG=n is set for disk caching
        if "RAM_CONFIG" not in config:
            config["RAM_CONFIG"] = "n"
        ...
        print(f"[Scanner] Config saved to: {config_path}")
        return True
    except Exception as e:
        print(f"[Scanner] WARNING: Failed to save config to disk: {e}")
        return False
...
def _maybe_apply_runtime_config_override() -> None:
    ...
    # Also save to disk so ConfigLoader can find it and enable disk cache
    _save_config_to_disk(base_config)
